...
func createDriverPod(app *v1beta2.SparkApplication, serviceLabels map[string]string, driverConfigMapName string, kubeClient ctrlClient.Client, appSpecVolumeMounts []apiv1.VolumeMount, appSpecVolumes []apiv1.Volume) error {
	...
	if templateFileExists {
		...
		if err != nil {
			fmt.Errorf("failed to load template file for the driver pod %s in namespace %s: %v", getDriverPodName(app), app.Namespace, err)
	...
	} else if checkSparkConf(app.Spec.SparkConf, config.SparkImagePullSecretKey) {
		imagePullSecretList, _ := app.Spec.SparkConf[config.SparkImagePullSecretKey]
	...
	} else if checkSparkConf(app.Spec.SparkConf, "spark.kubernetes.authenticate.driver.serviceAccountName") {
	...
	for sparkConfKey, sparkConfValue := range app.Spec.SparkConf {
		if strings.Contains(sparkConfKey, config.SparkDriverSecretKeyPrefix) {
			lastDotIndex := strings.LastIndex(sparkConfKey, DotSeparator)
...
func CreateDriverPodContainerSpec(app *v1beta2.SparkApplication) (apiv1.Container, []string) {
	...
	sparkConfigDir.Name = config.SparkConfDirEnvVar
	...
	} else if checkSparkConf(app.Spec.SparkConf, "spark.kubernetes.driver.container.image") {
		driverPodContainerSpec.Image = app.Spec.SparkConf["spark.kubernetes.driver.container.image"]
	} else if checkSparkConf(app.Spec.SparkConf, "spark.kubernetes.container.image") {
	...
	} else if checkSparkConf(app.Spec.SparkConf, "spark.kubernetes.container.image.pullPolicy") {
	...
	driverPodContainerSpec.Name = config.SparkDriverContainerName

	//Driver pod contianer ports
	driverPodContainerSpec.Ports = []apiv1.ContainerPort{
		{
			ContainerPort: int32(getDriverPort(sparkConfKeyValuePairs)),
			Name:          DriverPortName,
			Protocol:      Protocol,
		},
		{
...
func incorporateMemoryOvehead(memoryNumber int, app *v1beta2.SparkApplication, memoryUnit string) string {
	...
	var memory string
	if app.Spec.Driver.MemoryOverhead != nil || checkSparkConf(app.Spec.SparkConf, "spark.kubernetes.memoryOverhead") || checkSparkConf(app.Spec.SparkConf, "spark.driver.memoryOverhead") {
		...
		if app.Spec.MemoryOverheadFactor != nil {
		...
		} else if checkSparkConf(app.Spec.SparkConf, "spark.driver.memoryOverheadFactor") {
			...
			memoryOverheadFactor, _ = strconv.ParseFloat(getMemoryOverheadFactor(app), 64)
		}
...
func processSparkConfEnv(app *v1beta2.SparkApplication, driverPodContainerEnvVars []apiv1.EnvVar) ([]string, []apiv1.EnvVar) {
	...
	var driverPodContainerEnvVar apiv1.EnvVar
	for sparkConfKey, sparkConfValue := range app.Spec.SparkConf {
		if strings.Contains(sparkConfKey, config.SparkDriverSecretKeyRefKeyPrefix) {
			lastDotIndex := strings.LastIndex(sparkConfKey, DotSeparator)
...
func handleResources(app *v1beta2.SparkApplication) apiv1.ResourceRequirements {
	...
	} else if checkSparkConf(app.Spec.SparkConf, SparkDriverMemory) {
	...
	if app.Spec.Driver.CoreLimit != nil || checkSparkConf(app.Spec.SparkConf, config.SparkDriverCoreLimitKey) {
		var cpuQuantity resource.Quantity
		if app.Spec.Driver.CoreLimit != nil {
			cpuQuantity = resource.MustParse(*app.Spec.Driver.CoreLimit)
		} else {
			cpuQuantity = resource.MustParse(app.Spec.SparkConf[config.SparkDriverCoreLimitKey])
		}
	...
	if app.Spec.Driver.CoreRequest != nil {
		...
		cpuValExists = true
	} else if checkSparkConf(app.Spec.SparkConf, config.SparkDriverCoreRequestKey) {
		cpuQuantity = resource.MustParse(app.Spec.SparkConf[config.SparkDriverCoreRequestKey])
		cpuValExists = true
	} else if app.Spec.Driver.Cores != nil {
		cpuQuantity = resource.MustParse(fmt.Sprint(*app.Spec.Driver.Cores))
		cpuValExists = true
	} else if checkSparkConf(app.Spec.SparkConf, SparkDriverCores) {
