...
func createSparkAppConfigMap(app *v1beta2.SparkApplication, submissionID string, createdApplicationId string, kubeClient ctrlClient.Client, driverConfigMapName string, serviceName string) error {
	...
	driverConfigMapData[config.SparkAppNamespaceKey] = GetAppNamespace(app)

	// Utility function buildAltSubmissionCommandArgs to add other key, value configuration pairs
	driverConfigMapData[SparkPropertiesFileName], errorSubmissionCommandArgs = buildAltSubmissionCommandArgs(app, getDriverPodName(app), submissionID, createdApplicationId, serviceName)
...
func buildAltSubmissionCommandArgs(app *v1beta2.SparkApplication, driverPodName string, submissionID string, createdApplicationId string, serviceName string) (string, error) {
	...
	if app.Spec.MemoryOverheadFactor != nil {
		...
		args = args + fmt.Sprintf("%s=%s", config.SparkMemoryOverheadFactor, getMemoryOverheadFactor(app)) + NewLineString
	}
	...
	for key, value := range app.Spec.Driver.NodeSelector {
		args = args + fmt.Sprintf("%s%s=%s", config.SparkDriverNodeSelectorKeyPrefix, key, value) + NewLineString
	}
	for key, value := range app.Spec.Executor.NodeSelector {
		args = args + fmt.Sprintf("%s%s=%s", config.SparkExecutorNodeSelectorKeyPrefix, key, value) + NewLineString
	}
	...
	//
	args = args + fmt.Sprintf("%s=%v", SparkDriverPort, getDriverPort(sparkConfKeyValuePairs)) + NewLineString
	//
	args = populateAppSpecType(args, *app)

	args = args + fmt.Sprintf("%s=%v", SparkApplicationSubmitTime, time.Now().UnixMilli()) + NewLineString

	sparkUIProxyBase := ForwardSlash + GetAppNamespace(app) + ForwardSlash + app.Name
...
func populateComputeInfo(args string, app v1beta2.SparkApplication, sparkConfKeyValuePairs map[string]string) (string, error) {
	...
	} else if checkSparkConf(sparkConfKeyValuePairs, SparkDriverCores) {
	...
	} else if checkSparkConf(sparkConfKeyValuePairs, config.SparkDriverCoreRequestKey) {
		args = args + fmt.Sprintf("%s=%s", config.SparkDriverCoreRequestKey, sparkConfKeyValuePairs[config.SparkDriverCoreRequestKey]) + NewLineString
	}
	...
	} else if checkSparkConf(sparkConfKeyValuePairs, config.SparkDriverCoreLimitKey) {
		args = args + fmt.Sprintf("%s=%s", config.SparkDriverCoreLimitKey, sparkConfKeyValuePairs[config.SparkDriverCoreLimitKey]) + NewLineString
	}
	...
	} else if checkSparkConf(sparkConfKeyValuePairs, config.SparkExecutorCoreRequestKey) {
		args = args + fmt.Sprintf("%s=%s", config.SparkExecutorCoreRequestKey, sparkConfKeyValuePairs[config.SparkExecutorCoreRequestKey]) + NewLineString
	}
	return args, nil
}
func populateMemoryInfo(args string, app v1beta2.SparkApplication, sparkConfKeyValuePairs map[string]string) string {
	...
	} else if checkSparkConf(app.Spec.SparkConf, "spark.driver.memoryOverhead") {
	...
	} else if checkSparkConf(app.Spec.SparkConf, "spark.kubernetes.memoryOverhead") {
	...
	if app.Spec.Executor.Cores != nil {
		args = args + fmt.Sprintf("%s=%d", config.SparkExecutorCoreKey, *app.Spec.Executor.Cores) + NewLineString
	} else if checkSparkConf(sparkConfKeyValuePairs, config.SparkExecutorCoreKey) {
		val, _ := strconv.ParseInt(sparkConfKeyValuePairs[config.SparkExecutorCoreKey], 10, 32)
		args = args + fmt.Sprintf("%s=%d", config.SparkExecutorCoreKey, val) + NewLineString
	}
	...
	} else if checkSparkConf(sparkConfKeyValuePairs, config.SparkExecutorCoreLimitKey) {
		args = args + fmt.Sprintf("%s=%s", config.SparkExecutorCoreLimitKey, sparkConfKeyValuePairs[config.SparkExecutorCoreLimitKey]) + NewLineString
	}
	...
	} else if checkSparkConf(app.Spec.SparkConf, "spark.executor.memoryOverhead") {
