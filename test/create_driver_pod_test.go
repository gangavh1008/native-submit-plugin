...
func TestCreateSparkAppDriverPod(t *testing.T) {
	...
	testFn := func(test testcase, t *testing.T) {
		errCreateSparkAppConfigMap := nativesubmit.createSparkAppConfigMap(test.app, test.submissionID, test.createdApplicationId, fakeClient, test.driverConfigMapName, "test")
		if errCreateSparkAppConfigMap != nil {
			t.Errorf("failed to create configmap: %v", errCreateSparkAppConfigMap)
		}

		errCreateDriverPod := nativesubmit.createDriverPod(test.app, test.serviceLabels, test.driverConfigMapName, fakeClient, test.app.Spec.Driver.VolumeMounts, test.app.Spec.Volumes)
	...
	for index, test := range testcases {
		...
		if index == 1 {
			...
			test.app.Spec.Image = nil
			test.app.Spec.Driver.Image = stringPointer("871501607754.dkr.ecr.us-west-2.amazonaws.com/sfci/dva-transformation/spark-on-k8s-sample-apps/flowsnake-basic-operator-integration-spark-3.3.2:jenkins-dva-transformation-spark-on-k8s-sample-apps-spark-3.3.2-sfdc-3-itest")
			test.app.Spec.Driver.Memory = nil
			test.app.Spec.Driver.CoreLimit = nil
			test.app.Spec.Driver.Cores = nil
			test.app.Spec.MemoryOverheadFactor = stringPointer("0.15")
		...
		if index == 2 {
			...
			test.app.Spec.Driver.TerminationGracePeriodSeconds = nativesubmit.Int64Pointer(1)
		...
		if index == 3 {
			test.app.Spec.Driver.MemoryOverhead = stringPointer("10m")
		}
