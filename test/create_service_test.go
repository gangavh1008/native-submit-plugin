...
func TestCreateDriverService(t *testing.T) {
	...
	testFn := func(test testcase, t *testing.T) {
		errCreateSparkAppConfigMap := nativesubmit.createSparkAppConfigMap(test.app, test.submissionID, test.createdApplicationId, fakeClient, test.driverConfigMapName, "testservicename")
		...
		err := nativesubmit.createDriverService(test.app, serviceLabels, fakeClient, "abcdefg123231kkllkjjlkl", "testservicename")
		if err != nil {
			t.Errorf("failed to create driver service: %v", err)
		}
	}

	test.TestApp.Spec.SparkConf["spark.kubernetes.driver.service.label.test"] = "test-label"
	test.TestApp.Spec.SparkConf["spark.kubernetes.driver.service.label.testa"] = "test-label-a"
	testcases := []testcase{
		{
			app:                  test.TestApp,
